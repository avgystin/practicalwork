...
*   **Интеграция с Kafka:**
    *   Отправка сообщений в топик `postedmessages` при вызове эндпоинта `/post-message`.
...
|-------|----------|----------|
| `POST` | `/post-message` | Отправка сообщения в Kafka |
...
```
**7. Отправка сообщения в Kafka**
```
curl -X POST http://localhost:12222/post-message -H "Content-Type: application/json" -d "{\"msg_id\":\"test-123\"}"
```
Ответ:
```
OK
